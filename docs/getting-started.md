...
#### 第一步：打开终端，进入 mochibot 文件夹

```bash
cd mochibot
```

> 如果你忘了放在哪里——Windows 可以在文件管理器里找到 `mochibot` 文件夹，在地址栏输入 `cmd` 回车，就能在这个目录打开终端。
...
正常情况下会显示更新了哪些文件，或者 `Already up to date.`（已经是最新）。

**如果报错了？** 看下面的"遇到冲突怎么办"。

#### 第三步：更新依赖 + 重启

- **Windows**：双击 `setup.bat`（会自动更新依赖并打开管理后台）
...
python scripts/start.py                      # 启动 bot
```

#### 第四步：检查新配置项（可选）

新版本可能会新增配置项。你可以对比一下 `.env.example` 有没有新东西：
...
如果你手动改过 `mochibot` 文件夹里的代码文件（不是 `.env` 或 `data/`），`git pull` 可能会报错，类似：

```
error: Your local changes to the following files would be overwritten by merge
```

**解决方法**——把你的改动暂存起来，更新完再恢复：
